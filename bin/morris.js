...
(function() {
  var $_Jquery, Morris, compStyle, minutesSpecHelper, secondsSpecHelper,
    __slice = [].slice,
  ...
  function dateToUnix(dateData){
    return new Date(dateData).getTime()/1000;
  }
  
  function enableSeries(morrisGraph, morrisConfigObj) {
        function GraphFilterChart(morrisGraph, morrisConfigObj){
            ...
            this.orgElement     = morrisConfigObj.element;
            this.filterChartDate= null;
            this.orgXmin        = morrisGraph.xmin;
            this.orgXmax        = morrisGraph.xmax;
            this.morris         = morrisGraph;
            this.constructorName = morrisGraph.constructor.name;
        }

        var selfFilter = new GraphFilterChart(morrisGraph, morrisConfigObj);

        GraphFilterChart.prototype.initSelectYKeys = function() {
            /** set legend filter*/
            if (morrisConfigObj.makeLegend) {
                var keys                = selfFilter.origYKeys;
                var morrisListElement   = this.morris.options.morrisListElement;
                legendData              = "";
                if (this.morris.options.legendItem) {
                    $_Jquery.each( this.morris.options.legendItem, function(label,value ) {
                        legendData += "<li class='"+morrisListElement+" disablePointer' style='color:#626e82'><i style='background-color:#626e82'>&nbsp;</i>" + label + ": " + value + "</li>";
                    });
                } else {
                    for (j = _i = 0, _len = this.morris.options.labels.length; _i < _len; j = ++_i) {
                        if (!this.morris.options.labels[j])
                            continue;
                        legendData += "<li class='"+morrisListElement+"' data-key-name=\"" + this.morris.options.labels[j] + "\" style='color: " + (this.morris.colorFor(j, j, 'label')) + "'><i style='background-color:" + (this.morris.colorFor(j, j, 'label')) + "'>&nbsp;</i>" + this.morris.options.labels[j] + "</li>";
                    }
                }
                this.morris.elJquery.after("<div class='"+this.morris.options.morrisLegendContainer+"' data-parent-graph=\"" + this.container.id + "\">" + legendData + "</div>");
                this.morris.elJquery.parent().find('.'+morrisListElement).bind("click", function() {
                    clickedKeyName = $_Jquery(this).attr("data-key-name");
                    if (clickedKeyName) {
                        selfFilter.toggleYKeys(new Array(clickedKeyName));
                        $_Jquery(this).toggleClass('deactivate');
                    }
                });
            }
            /** set slider filter*/
            if($_Jquery.inArray(this.constructorName, ['Area','Line']) != -1 && morrisConfigObj.rangeFilterSlider){
                this.morris.elJquery.parent().find('.'+this.morris.options.morrisChartSlider).slider(
                    {
                        range: true,
                        min: selfFilter.orgXmin,
                        max: selfFilter.orgXmax,
                        values: [selfFilter.orgXmin,selfFilter.orgXmax ],
                        slide: function(event,rangeData) {
                            
                        },
                        stop: function(event, rangeData) {
                            selfFilter.rangeSelectCall({start: rangeData.values[0],end: rangeData.values[1]});
                        }
                    }
                );
            }
        ...
        GraphFilterChart.prototype.toggleYKeys = function(yKeysToToggle) {
            ...
            this.morris.options.animate             = true;
            this.morris.options.filterYlabelDisplay = true;
            this.setYKeys(newYKeys);
        };
        
        GraphFilterChart.prototype.rangeSelectCall = function(graphFilters) {
            this.filterChartDate = graphFilters;
            this.morris.options.animate                 = false;
            this.morris.options.filterYlabelDisplay     = true;
            this.setYKeys(this.displayedYKeys);
        };
        
        GraphFilterChart.prototype.setYKeys = function(yKeys) {
        ...
        GraphFilterChart.prototype.filterData = function(data, yKeysToShow) {
            var resultObj   = [];
            var adasdadas   = [];
            var mainXkey    = ($_Jquery.isArray(this.xkey)) ? this.xkey[0] : this.xkey;
            for (var key in data) {
                    ...
                    for (var innerObjKey in innerObj) {
                        if (innerObj.hasOwnProperty(innerObjKey)) {
                            for (var i = 0, size = yKeysToShow.length; i < size; i++) {
                                //if (innerObjKey === this.xkey || ($_Jquery.isArray(this.xkey) && $_Jquery.inArray(innerObjKey, this.xkey) != -1) || innerObjKey === yKeysToShow[i]) {
                                if (innerObjKey === mainXkey || innerObjKey === yKeysToShow[i]) {
            ...
            if(this.filterChartDate){
                var endDate     = this.filterChartDate.end/1000;
                var startDate   = this.filterChartDate.start/1000;
                for (var newIndexKey in resultObj) {
                    var dataDate = new Date(resultObj[newIndexKey][mainXkey]).getTime()/1000;
                    if(!(dataDate >= startDate && dataDate <= endDate) ){
                        delete resultObj[newIndexKey];
                    }
                }
                resultObj = resultObj.filter(function(val){return val});
            }
            return resultObj;
        };
        selfFilter.initSelectYKeys();
        return selfFilter;
    }
  ...
  Morris.Grid = (function(_super) {
    ...
    function Grid(options) {
      ...
      $_Jquery(document.getElementById(this.options.element)).append("<div class='"+this.options.morrisChartSlider+"'></div>");
      $_Jquery(document.getElementById(this.options.element)).append("<div id='"+this.options.element+this.options.morrisContainerId+"' class='"+this.options.morrisContainerClass+"'></div>");
      //$_Jquery(document.getElementById(this.options.element)).css('height','auto');
      this.options.element  = this.options.element+this.options.morrisContainerId;
      if (typeof this.options.element === 'string') {
        this.el         = document.getElementById(this.options.element);
        this.elJquery   = $_Jquery(document.getElementById(this.options.element));
      } else {
        this.el         = this.options.element[0] || this.options.element;
        this.elJquery   = $_Jquery(this.options.element);
      }
      if (this.el == null) {
      ...
      if (this.options.rangeSelect) {
        this.selectionRect = this.raphael.rect(0, 0, 0, Morris.innerDimensions(this.el).height*70/100).attr({
      ...
      this.options.GraphFilterChart = enableSeries(this, options);
    ...
    Grid.prototype.gridDefaults = {
      ...
      /** custom options*/
      makeLegend:           false,
      morrisContainerId:    'MorrisContainer',
      morrisChartSlider:    'morrisChartSlider',
      morrisContainerClass: 'morrisChartContainer',
      morrisLegendContainer:'morrisLegendContainer',
      morrisListElement:    'toogleseries'
    };
    ...
    Grid.prototype.setData = function(data, redraw) {
      ...
      this.data = (function() {
        ...
        for (index = _i = 0, _len = data.length; _i < _len; index = ++_i) {
          ...
          xkeyvalues.push(dateToUnix(row[this.options.xkey]));            
    ...
    Grid.prototype.yLabelFormat = function(label, i) {
      ...
      }else if (this.options.yLabelFormat === 'numberBeautifier') {
        return this.numberBeautifier(label, i);
      } else {
    ...
    Grid.prototype.numberBeautifier = function(label, index,digits = 1) {
        ...
        for (i = si.length - 1; i > 0; i--) {
            if (label >= si[i].value) {break;}
        }
        var labelName = (this.options.postUnits) ? this.options.postUnits : si[i].symbol;
        return (label / si[i].value).toFixed(digits).replace(rx, "$1") + " " + labelName;
  };
    ...
    Grid.prototype.endRange = function(x) {
      ...
      if (this.selectFrom) {
        ...
        if (typeof this.options.GraphFilterChart.rangeSelectCall === "function")
        {
  ...
  Morris.Line = (function(_super) {
    ...
    Line.prototype.hoverContentForRow = function(index) {
      ...
      for (_l = order.length - 1; _l >= 0; _l += -1) {
        ...
        if((row.y[j] && this.options.filterYlabelDisplay) || !this.options.filterYlabelDisplay){
            labelValue = (this.options.yLabelFormat === 'numberBeautifier') ? row.y[j] : this.yLabelFormat(row.y[j], j);
  ...
  Morris.Bar = (function(_super) {
    ...
    Bar.prototype.hoverContentForRow = function(index) {
      ...
      for (jj = _j = 0, _len1 = inv.length; _j < _len1; jj = ++_j) {
        ...
        if((y && this.options.filterYlabelDisplay) || !this.options.filterYlabelDisplay){
            labelValue = (this.options.yLabelFormat === 'numberBeautifier') ? y : this.yLabelFormat(y, j);
            content += "<div class='morris-hover-point' style='color: " + (this.colorFor(row, j, 'label')) + "'>\n  " + this.options.labels[j] + ":\n  " + labelValue + "\n</div>";
        }
      }
      if (typeof this.options.hoverCallback === 'function') {
